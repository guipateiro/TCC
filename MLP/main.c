...
//Get Labels
void get_desired_outputs(FILE *inp)
{
    int i,j;
    ...
    for(i=0;i<num_training_ex;i++)
    {
        for(j=0;j<num_neurons[num_layers-1];j++)
        {
            ...
            fscanf(inp,"%f",&desired_outputs[i][j]);
            printf("\n");
        }
    }
}
