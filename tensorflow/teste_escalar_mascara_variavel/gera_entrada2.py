...
for i in range(0, tam):
    ...
    binario1 = binario + rand1 + rand2 + rand3 + rand4 + rand5 + uns
    ...
    rando = []
    rando.append(format(gerar_numero_aleatorio(), '016b'))
    rando.append(format(gerar_numero_aleatorio(), '016b'))
    rando.append(format(gerar_numero_aleatorio(), '016b'))
    rando.append(format(gerar_numero_aleatorio(), '016b'))
    rando.append(format(gerar_numero_aleatorio(), '016b'))
    binarioaux = binario
    for k in range(parametro):
        binarioaux = binarioaux + rando[k]
    for j in range(parametro,5):
        binarioaux = binarioaux + rand   
    binarioaux = binarioaux + zeros
    binario_e = ' '.join(binarioaux)   
    fi.write(binario_e)
    fi.write('\n')
    fs.write("%f" % 1)   
    fs.write('\n') 
