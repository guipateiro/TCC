...
nome_arq_precisao = "data2/precisao_" + parametro + ".txt"
fi = open(nome_arq_precisao, "a")
fi.write(f"RUN:\n")
for i in range(19):
    subprocess.run(["python.exe", "modifica_entrada.py", str(i)])
    X_test = []
    with open("input_data_modificado.txt", 'r') as input_file:
        lines = input_file.readlines()
        numeric_lists = []
        for line in lines:
            numbers = line.split()
...
nome_arq_pesos = "data2/pesos_" + parametro + ".txt"
